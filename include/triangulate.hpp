...
namespace geometry {
    ...
    template<class T> bool Mate(const Edge<T> & e, const std::vector<Point<T>> & v,
                                Point<T> & p) {
    ...
    template<class T> Edge<T> HullEdge(std::vector<Point<T>> & v) {
        long unsigned m = 0;
        for (long unsigned i = 1; i < v.size(); i++) if (v[i] < v[m]) m = i;
        std::swap(v[0], v[m]);
        m = 1;
        for (long unsigned i = 2; i < v.size(); i++) {
    ...
    template<class T> int SetFind(const std::vector<T> & frontier, const T & t) {
        for (long unsigned i = 0; i < frontier.size(); i++) if (frontier[i] == t) return int(i);
        return -1;
    }
    template<class T> void SetAdd(std::vector<T> & frontier, const T & t) {
        auto iter = frontier.begin();
        for (long unsigned i = 0; i < frontier.size(); i++) 
            if (frontier[i] >= t) {
    ...
    template<class T> void UpdateFrontier(std::vector<Edge<T>> & frontier, const Point<T> & a, const Point<T> & b) {
        Edge<T> e(a, b);
    ...
    template<class T> int PointIndex(const std::vector<Point<T>> & v, const Point<T> & p) {
        for (long unsigned i = 0; i < v.size(); i++) if (Point<T>::IsEqual(v[i], p, T(1.0E-8))) return int(i);
        throw p;
    }
    template<class T> std::vector<int> Triangulate(const std::vector<Point<T>> & v) {
